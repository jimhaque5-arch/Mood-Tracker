  ...
  return (
    <div className="min-h-screen bg-[#07070f] text-white">
      ...
      <header className="sticky top-0 z-50 border-b border-white/5 bg-black/30 backdrop-blur-md">
        <div className="max-w-lg mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <motion.div animate={{ rotate: [0, 12, -12, 0] }} transition={{ duration: 2.5, repeat: Infinity, repeatDelay: 4 }} className="text-2xl">
              🌈
            </motion.div>
            <div>
      ...
      <main className="relative max-w-lg mx-auto px-4 py-6 space-y-5">
        {/* Hero */}
        {!isConnected && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="text-center py-8 space-y-4">
            <motion.div animate={{ scale: [1, 1.04, 1] }} transition={{ duration: 2.5, repeat: Infinity }} className="text-6xl">
              😄🎭🌟
            </motion.div>
            <div>
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {["⛓️ Base Mainnet", "🔐 Non-Custodial", "📊 Community Stats", "🔥 Streak System"].map((tag) => (
                <span key={tag} className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-gray-400 text-xs">{tag}</span>
              ))}
            </div>
          </motion.div>
        )}
          ...
          {TABS.map((tab) => (
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${
                activeTab === tab.id ? "bg-indigo-500 text-white shadow-lg shadow-indigo-500/20" : "text-gray-500 hover:text-gray-300 hover:bg-white/5"
              }`}
            >
              <span>{tab.emoji}</span>
        ...
        {/* Content */}
        <motion.div key={activeTab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.18 }} className="rounded-3xl bg-white/[0.04] border border-white/10 backdrop-blur-sm p-6">
          {activeTab === "log" && <MoodSelector key={refreshKey} hasSubmittedToday={!!hasSubmittedToday} todayMoodId={todayMoodId} onSuccess={handleSuccess} />}
          {activeTab === "history" && <div><h3 className="text-base font-bold text-white mb-4">📅 Your Mood Journey</h3><MoodHistory key={refreshKey} /></div>}
          {activeTab === "community" && <div><h3 className="text-base font-bold text-white mb-4">🌍 Community Vibes</h3><GlobalStats /></div>}
        </motion.div>

        <p className="text-center text-gray-700 text-xs pb-4">
          Built on <a href="https://base.org" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-400">Base</a> · Your moods, your keys 🔑
        </p>
      </main>
    </div>
  );
